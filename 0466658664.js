...
(function () {
  ...
  function setMode(isEditing) {
    if (!editorMode || !publishBtn || !cancelEditBtn) return;
  ...
  function clearForm() {
    ...
    if (coverImageFilesEl) coverImageFilesEl.value = "";
  ...
  inlineImageInput?.addEventListener("change", async () => {
    ...
    try {
      ...
      console.error(err);
      setStatus(publishStatus, "插图上传失败：" + (err.message || err));
    } finally {
  ...
  async function saveArticle() {
    const supabase = getSupabase();
    if (!supabase) {
      setStatus(publishStatus, "Supabase 未初始化，请先检查 supabase.js 和脚本顺序。");
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setStatus(publishStatus, "保存失败：" + (err.message || err));
    }
  }
  ...
  async function startEdit(id) {
    const supabase = getSupabase();
    if (!supabase) {
      alert("Supabase 未初始化，请检查 supabase.js 是否先加载。");
      return;
    }
    ...
    try {
      ...
      if (editorEl) editorEl.innerHTML = data.html_body || data.content || "";
  ...
  async function loadNews() {
    ...
    if (!supabase) {
      ...
      emptyHint.textContent = "Supabase 未初始化，请检查 HTML 脚本顺序。";
      return;
    }
    ...
    try {
      ...
      data.forEach((row) => {
        ...
        delBtn.addEventListener("click", async () => {
          ...
          try {
            const { error } = await supabaseInner.from("news").delete().eq("id", id);
            if (error) throw error;
      ...
      console.error(err);
      emptyHint.style.display = "block";
      emptyHint.textContent = "加载失败：" + (err.message || err);
    }
  }
